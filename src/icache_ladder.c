...
#define FNAME "icache_ladder_out.file"
...
// TODO: Make this use the arbitrarily long ladder
// 1. store a copy of the original
// 2. test with a copy
...
void gadget_init(gadget_t* gadget, uint64_t* t1, uint64_t* t2, FILE* f)
{
  ...
  memcpy(gadget->backup, gadget_start, size_gadget());
#if VERBOSE
  fprintf(stderr, DBG "MEMORY INIT\n" COLOR_RESET);
#endif
